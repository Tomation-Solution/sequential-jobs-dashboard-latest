...
import ApplicantsDeleteAccountModal from '../modals/applicants/ApplicantsDeleteAccountModal'
import { ApplicantsTableDataType } from '../../types/tables/applicants'
// import DeleteAccountModal from '../modals/DeleteAccountModal'
import ApplicantsTableDropDown from '../dropdowns/ApplicantsTableDropDown'
import { applicantsData } from '../../data/table/applicantsTableData'

  const columnHelper = createColumnHelper<ApplicantsTableDataType>()
  ...
  const columns = [
    ...
    columnHelper.accessor('numberOfApplications', {
        header:'No. of Applications',
      cell: info => info.getValue(),
    }),
    columnHelper.accessor('cv', {
        header:'CV',
      cell: cv => <span className='text-green-500' >{cv.getValue()}</span>,
    }),
    columnHelper.accessor('registrationDate', {
        header:'Registration Date',
      cell: registrationDate => <span className='text-[#DC9025'>{registrationDate.getValue()}</span>,
    }),
...
const ApplicantsTable = () => {
    ...
    const [data, ] = React.useState(() => [...applicantsData])
    const [activeRow, setActiveRow] = React.useState<null|number>(null)
    const [isApplicantsDropDownOpen,setIsApplicantsDropdownOpen] =  React.useState<null|number>(null) // this checks if the company dropdown is active
...
<div className="p-2 registration-table">
          ...
          >
            {row.getVisibleCells().map(cell => (
             ...
             <td className='hover:cursor' >
              ...
              <SlOptions key={row.original.id} onClick={() => (isApplicantsDropDownOpen === row.original.id ? setIsApplicantsDropdownOpen(null) : setIsApplicantsDropdownOpen(row.original.id))}   className='w-5 h-5 text-md text-gray-300 hover:cursor-pointer'  />
              {isApplicantsDropDownOpen === row.original.id ? <ApplicantsTableDropDown key={row.original.id} handleOpenModal={handleOpenModal}  currentRow={row.original}  /> : ''}            
            </td>
          </tr>
        ))}
      </tbody>
    ...
    <div>
        {modalTypeActive === 'delete-applicant-account-modal' && <ApplicantsDeleteAccountModal selectedRowData={selectedRowData} handleCloseModal={handleCloseModal} />}
    </div>
</>
  )
}
